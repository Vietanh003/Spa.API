...
export const dvApi = {
  list: async (params: { active?: boolean; danhMucId?: number; q?: string }) => {
    ...
    const url = sp.toString() ? `/api/admin/dich-vu?${sp}` : "/api/admin/dich-vu";
    return (await api.get<DichVuRow[]>(url)).data;
  },
  get: async (id: number) => (await api.get<DichVuDetail>(`/api/admin/dich-vu/${id}`)).data,
  create: async (dto: CreateDto) => (await api.post("/api/admin/dich-vu", dto)).data,
  update: async (id: number, dto: UpdateDto) => (await api.put(`/api/admin/dich-vu/${id}`, dto)).data,
  remove: async (id: number) => (await api.delete(`/api/admin/dich-vu/${id}`)).data,
};
