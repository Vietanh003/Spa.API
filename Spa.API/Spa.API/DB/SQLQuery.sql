...
-- =============================================
-- Bảng LichHen - Quản lý đặt lịch hẹn Spa
-- =============================================
...
BEGIN
    ...
    (
        ...
        -- Thông tin khách hàng (không yêu cầu đăng nhập)
        HoTenKhach          NVARCHAR(200)     NOT NULL,
        DienThoaiKhach      NVARCHAR(20)      NOT NULL,
        EmailKhach          NVARCHAR(100)     NULL,

        -- Thông tin lịch hẹn
        NgayHen             DATE              NOT NULL,           -- Ngày hẹn (không giờ)
        GioHen              TIME(0)           NOT NULL,           -- Giờ hẹn (ví dụ: 09:30:00)
        ThoiLuongDuKien     INT               NOT NULL            -- Thời lượng dự kiến (phút)

        CONSTRAINT CK_LichHen_ThoiLuongDuKien 
            CHECK (ThoiLuongDuKien > 0),

        -- Liên kết với Nhân viên thực hiện (nếu đã phân công)
        NhanVienId          INT               NULL,
        ...
        -- Thông tin bổ sung
        GhiChuKhach         NVARCHAR(500)     NULL,   -- Ghi chú từ khách hàng
        GhiChuNoiBo         NVARCHAR(1000)    NULL,   -- Ghi chú nội bộ (nhân viên spa)

        -- Trạng thái lịch hẹn
        TrangThai           NVARCHAR(50)      NOT NULL 
            CONSTRAINT DF_LichHen_TrangThai DEFAULT (N'Chờ xác nhận'),
        -- Các trạng thái phổ biến: Chờ xác nhận, Đã xác nhận, Đang thực hiện, Hoàn thành, Hủy, Không đến
        ...
        CreatedAt           DATETIME2(0)      NOT NULL 
            CONSTRAINT DF_LichHen_CreatedAt DEFAULT (SYSDATETIME()),
        UpdatedAt           DATETIME2(0)      NULL,
        CreatedBy           NVARCHAR(128)     NULL,     -- Có thể lưu DbLoginName của nhân viên tạo lịch
        UpdatedBy           NVARCHAR(128)     NULL,

        -- Khách có đến không (để thống kê)
        DaDen               BIT               NOT NULL 
            CONSTRAINT DF_LichHen_DaDen DEFAULT (0)
    );

    -- ==================== INDEXES ====================

    -- Index chính cho tra cứu nhanh theo số điện thoại khách (rất quan trọng)
    CREATE NONCLUSTERED INDEX IX_LichHen_DienThoaiKhach 
        ON dbo.LichHen(DienThoaiKhach);

    -- Index theo ngày hẹn + giờ (hiển thị lịch theo ngày)
    CREATE NONCLUSTERED INDEX IX_LichHen_NgayGioHen 
        ON dbo.LichHen(NgayHen, GioHen);

    -- Index theo trạng thái (lọc nhanh lịch chờ xác nhận, đã xác nhận...)
    CREATE NONCLUSTERED INDEX IX_LichHen_TrangThai 
        ON dbo.LichHen(TrangThai);

    -- Index theo dịch vụ
    CREATE NONCLUSTERED INDEX IX_LichHen_DichVuId 
        ON dbo.LichHen(DichVuId);

    -- Index theo nhân viên được phân công
    CREATE NONCLUSTERED INDEX IX_LichHen_NhanVienId 
        ON dbo.LichHen(NhanVienId);

    -- Index hỗ trợ xem lịch của một khách hàng theo thời gian
    CREATE NONCLUSTERED INDEX IX_LichHen_DienThoai_NgayHen 
        ON dbo.LichHen(DienThoaiKhach, NgayHen DESC);

    -- Index tổng hợp cho dashboard (ngày + trạng thái)
    CREATE NONCLUSTERED INDEX IX_LichHen_Ngay_TrangThai 
        ON dbo.LichHen(NgayHen, TrangThai);

    -- ==================== FOREIGN KEYS ====================

    -- Liên kết với Nhân viên
    ALTER TABLE dbo.LichHen
        ADD CONSTRAINT FK_LichHen_NhanVien 
        FOREIGN KEY (NhanVienId) REFERENCES dbo.NhanVien(Id);

    -- Liên kết với Dịch vụ
    ALTER TABLE dbo.LichHen
        ADD CONSTRAINT FK_LichHen_DichVu 
        FOREIGN KEY (DichVuId) REFERENCES dbo.DichVu(Id);
